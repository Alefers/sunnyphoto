...
<html>
   <head>
      ...
      <link rel="stylesheet" href="design/album.css" type="text/css" />
...
?>
      ...
      <div id="content">
<?php
$files = scandir('images/articles');
$line = array(0, 0, 0);
for ($i = 2; $i < count($files); $i++) {
   $imgsize = getimagesize('images/articles/' . $files[$i]);
   $coef = $imgsize[1]/$imgsize[0];
   if ($line[0] <= $line[1]) {
      if ($line[0] <= $line[2]) {
         $line[0] += $coef;
         $lines[0][] = $files[$i];
...
for ($n = 0; $n < 3; $n++) {
   ...
   foreach ($lines[$n] as $value) {
      echo '<img src="images/articles/' . $value . '">';
   }
   echo '</div>';
}
?>
